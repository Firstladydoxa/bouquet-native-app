...
export const useSubscriptionService = (): SubscriptionService => {
  ...
  const service = useMemo<SubscriptionService>(() => {
    ...
    const isPromotionalPeriodActive = (): boolean => {
      const promotionEndDate = new Date('2026-12-31T23:59:59');
      const now = new Date();
    ...
    const isFreeTrialActive = (): boolean => {
      ...
      const trialEndDate = new Date(FREE_TRIAL_DATES.END_WITH_TIME);
      const now = new Date();
      ...
      if (language.type === 'subscription') {
        ...
        if (category === 'free_trial' && isFreeTrialActive()) {
          return {
            ...
            shouldShowPurchaseOption: false,
            shouldShowFreeTrialOption: false,
            message: `You have access through your free trial (ends ${FREE_TRIAL_DATES.DISPLAY_TEXT})`,
            actionText: undefined
          };
        }
        ...
        if (category === 'free') {
          if (isEligibleForFreeTrial()) {
            return {
              ...
              message: `This is a premium language, but there's a promotional Free Trial running now till ${FREE_TRIAL_DATES.DISPLAY_TEXT} that gives access to all free + premium languages.`,
